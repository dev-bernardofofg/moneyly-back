...
export const createUser = asyncHandler(async (req: Request, res: Response) => {
  const { user, accessToken, refreshToken } = await userService.createUser(req.body);
...
export const createSession = asyncHandler(async (req: Request, res: Response) => {
  const { user, accessToken, refreshToken } = await userService.createSession(req.body);
...
export const createGoogleSession = asyncHandler(async (req: Request, res: Response) => {
  ...
  const { user, accessToken, refreshToken } = await userService.createGoogleSession(idToken);
  return ResponseHandler.success(
    res,
...
export const refreshToken = asyncHandler(async (req: Request, res: Response) => {
  ...
  const { user, accessToken } = await userService.refreshToken(refreshTokenValue);
  return ResponseHandler.success(
    res,
...
export const logout = asyncHandler<AuthRequest>(async (req, res) => {
  ...
  await userService.revokeRefreshToken(req.user.id, refreshTokenValue);
