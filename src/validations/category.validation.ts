...
export const validateCategoryExistsByUserId = async (id: string, userId: string) => {
...
export const validateCategoryIsNotGlobal = async (id: string, userId: string) => {
  const category = await categoryRepository.findByIdAndUserId(id, userId);
...
export const validateCategoryNameIsNotInUse = async (name: string, userId: string) => {
...
export const validateHideGlobalCategory = async (id: string, userId: string) => {
  const category = await categoryRepository.hideGlobalCategoryForUser(userId, id);
  if (!category) {
