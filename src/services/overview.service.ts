...
// ——— Serviços com dependências: factory ———

export const makeOverviewService = (deps: OverviewServiceDeps) => {
  const {
  ...
  const getAvailablePeriods = async (userId: string) => {
    const storedPeriods = await financialPeriodRepository.findAllByUserWithTransactionCount(userId);
  ...
  const getFinancialInsights = async (userId: string, monthlyIncome: number) => {
  ...
  const getPlannerOverview = async (userId: string, monthlyIncome: number) => {
    const budgetProgress = await getBudgetProgress(userId);
    const goalsProgress = await getGoalsProgress(userId);
...
export const overviewService = makeOverviewService({
  financialPeriodRepository,
  transactionRepository,
  userRepository,
