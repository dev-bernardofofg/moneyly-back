...
test.describe('Fluxo de Autenticação E2E', () => {
  ...
  test('deve permitir cadastro completo de novo usuário', async ({ request }) => {
    // Criar novo usuário
  ...
  test('deve rejeitar acesso a rotas protegidas sem token', async ({ request }) => {
    const response = await request.get(`${API_BASE_URL}/transactions/summary`);

    expect(response.status()).toBe(401); // Unauthorized
  });

  test('deve permitir acesso a rotas protegidas com token válido', async ({ request }) => {
    const response = await request.get(`${API_BASE_URL}/transactions/summary`, {
      headers: {
