...
import { createSaoPauloDate } from '@core/helpers/dates';
import {
...
describe('FinancialPeriodHelper', () => {
  ...
  describe('isDateInCurrentFinancialPeriod', () => {
    it('returns true when the date is within the current period', () => {
      ...
      const result = isDateInCurrentFinancialPeriod(testDate, 5, 25, referenceDate);
  ...
  describe('getAvailableFinancialPeriods', () => {
    ...
    it('handles periods that cross years', () => {
      ...
      const periods = getAvailableFinancialPeriods(1, 31, transactionsAcrossYears);
  ...
  describe('getPreviousFinancialPeriods', () => {
    it('includes the current period as the first item', () => {
      ...
      expect(firstPeriod).toBeDefined();
      if (firstPeriod) {
        expect(firstPeriod.startDate.getTime()).toBe(currentPeriod.startDate.getTime());
        expect(firstPeriod.endDate.getTime()).toBe(currentPeriod.endDate.getTime());
      }
    });
