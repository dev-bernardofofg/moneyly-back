...
import { generateOpenApiDocument } from '@core/openapi/generate';
...
describe('openapi generator', () => {
  ...
  it('public routes require no auth; private routes require bearer', () => {
    ...
    expect(doc.paths?.['/overview/insights']?.get?.security).toEqual([{ bearerAuth: [] }]);
  ...
  it('data references $ref (no longer generic {})', () => {
    ...
    const dataSchema =
      d.paths['/overview/insights'].get.responses['200'].content['application/json'].schema
        .properties.data;
  ...
  it('router↔openapi regression: every router-declared endpoint is in the doc', () => {
    const missing = scanRouters().filter(
      (r) => !(doc.paths as Record<string, Record<string, unknown>>)?.[r.path]?.[r.method]
    );
